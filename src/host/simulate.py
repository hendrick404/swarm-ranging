...
class Node:
    ...
    def rx(self, global_time, message, pos):
        ...
        json_obj = {"id": self.id, "rx range": {"sender id": message["id"], "seq num": message["tx range"]["seq num"], "tx time": message["tx range"]["tx time"], "rx time": int(rx_time)}}
        return json_obj
...
def main():
    ...
    while exchange_counter <= 100 * len(nodes):
        for tx_node in nodes:
            ...
            tx_message = tx_node.tx(global_clock)
            eval_file.write(json.JSONEncoder().encode(tx_message) + "\n")
            for rx_node in nodes:
                if rx_node != tx_node:
                    rx_message = rx_node.rx(global_clock, tx_message, tx_node.get_pos())
