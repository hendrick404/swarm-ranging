...
class Node:
    ...
    def rx(self, global_time, message, pos, message_receive_timestamps):
        rx_time = (global_time + (sqrt((self.pos[0] - pos[0]) ** 2 + (self.pos[1] - pos[1]) ** 2) / (speed_of_light / second))) * self.clock_err + self.clock_offset
        self.receive_timestamps[message["id"]] = (message["tx range"]["seq num"], int(rx_time))
        rx_timestamps = []
        for (id, (seq_num, rx_timestamp)) in message_receive_timestamps.items():
            rx_timestamps.append({"id": id, "seq num": seq_num, "rx time": rx_timestamp})
